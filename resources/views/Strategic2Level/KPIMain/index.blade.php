...
<div class="main_container">
    ...
    <div role="main">
        <div class="">
            ...
            @include('Strategic2Level.KPIMain.create')
                            ...
                            <table id="example" class="display">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($sortedKPI as $item )
                                        <tr>
                                            <th scope="row">{{$i}}</th>
                                            <td>{{ $item->SFA->strategic->year->year ?? 'ไม่พบปี' }}</td>
                                            <td>{{ $item->SFA->strategic->name ?? 'ไม่พบแผนยุทธศาสตร์' }}</td>
                                            <td>{{ $item->SFA->name ?? 'ไม่พบประเด็นยุทธศาสตร์' }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->count ?? 'ไม่มีหน่วยนับ' }}</td>
                                            <td>{{ $item->target ?? 'ไม่มีค่าเป้าหมาย'}}</td>
                                            <td>{{ $item->director->displayname ?? 'ไม่มีผู้กำกับ'}}</td>
                                            <td>{{ $item->recorder->displayname ?? 'ไม่มีผู้บันทึกข้อมูล'}}</td>
                                            <td>
                                                <a
                                                        href="{{route('KPIMain2LV.edit',$item->KPIMain2LVID)}}"><i
                                                            class="fa fa-pencil btn btn-warning"></i></a>
                                                    <a href="{{route('KPIMain2LV.delete',$item->KPIMain2LVID)}}"
